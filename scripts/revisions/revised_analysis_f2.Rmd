...
fee_fig <- fee %>%
  ...
  dplyr::select(Category,carbon_vulnerable,richness_all:RSR_all, area, Date_Est) %>%
  dplyr::rename(`Species Richness` = richness_all,
         `Range-size rarity` = RSR_all,
         `CRENVU richness` =  richness_crenvu)
```

```{r fig.width=3, fig.height=6, dpi= 300}
fig2b <- easements %>%
  dplyr::select(Category,carbon_vulnerable,richness_all:RSR_all, Date_Est, area) %>%
  dplyr::rename(`Species Richness` = richness_all,
         `Range-size rarity` = RSR_all,
         `CRENVU richness` =  richness_crenvu) %>%
  drop_na() %>%
  ...
  drop_na() %>%
  dplyr::select(-c(Date_Est, area, carbon_vulnerable)) %>%
...
fee_fig_C <- fee_fig %>%
  dplyr::select(Category,carbon_vulnerable, Date_Est, area)
...
fig3b <- easements %>%
  ...
  theme(legend.position = "none") +
  theme(axis.title.x = element_blank()) +
  labs(y = "mean vulnerable carbon density \n (MgC) per parcel")
fig3b
```
...
ag_b <- background %>%
  ...
  summarise(richness_all_b = sum(richness_all, na.rm = TRUE),
         RSR_all_b = sum(RSR_all, na.rm = TRUE),
...
fig2a <- ag_e %>%
  ...
  dplyr::mutate(`Species Richness` = (richness_all-richness_all_b)/richness_all_b,
                ...
                `CRENVU Richness` = (richness_crenvu-richness_crenvu_b)/richness_crenvu_b,
                carbon_vulnerable = (carbon_vulnerable-carbon_vulnerable_b)/carbon_vulnerable_b) %>% 
  dplyr::select(Category,`Species Richness`:`CRENVU Richness`) %>%
  pivot_longer(-Category) %>%
...
```{r fig2, fig.width=6, fig.height=6, dpi = 300}
library(patchwork)
fig2a + fig2b
```
# Carbon

```{r carbon-figure, fig.height = 3, fig.width = 6, dpi = 300}
public_private_carbon <- f2public %>%
  rename(area = total_area_public) %>%
  rbind(f2private) %>%
  mutate(bind= "a") %>%
  full_join(ag_b) %>%
  dplyr::mutate(richness_all = (richness_all-richness_all_b)/richness_all_b,
                RSR_all = (RSR_all-RSR_all_b)/RSR_all_b,
                richness_crenvu = (richness_crenvu-richness_crenvu_b)/richness_crenvu_b,
                carbon_vulnerable = (carbon_vulnerable-carbon_vulnerable_b)/carbon_vulnerable_b) %>%
  dplyr::select(carbon_vulnerable, Category) %>%
  pivot_longer(-Category) %>%
  mutate(type = Category) %>%
  mutate(Category = plyr::revalue(Category, c(Private = "easement", Public = "fee") ))
...
fig3a <- ag_e %>%
  ...
  dplyr::mutate(richness_all = (richness_all-richness_all_b)/richness_all_b,
                ...
                richness_crenvu = (richness_crenvu-richness_crenvu_b)/richness_crenvu_b,
                carbon_vulnerable = (carbon_vulnerable-carbon_vulnerable_b)/carbon_vulnerable_b) %>% 
  dplyr::select(Category,carbon_vulnerable) %>%
  pivot_longer(-Category) %>%
  ggplot(aes(x = Category, y = value*100, fill = Category)) + 
  geom_bar(stat = "identity") +
    scale_fill_manual(values=c("#0072B2", "#E69F00", "black", "black")) +
  geom_point(public_private_carbon, mapping = aes(x = Category, y= as.numeric(value)*100,  
                                           group = type, 
                                           shape= type), stat = "identity", 
             position=position_dodge(width=0.8), size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed") +
